...
#            loss = cifar10.loss(logits, labels, batch_size)
            ...
            # Decay the learning rate exponentially based on the number of steps.
                lr = tf.train.exponential_decay(INITIAL_LEARNING_RATE*len(worker_hosts),global_step,
                    decay_steps,
...
#	    sess.run(tf.global_variables_initializer())
            ...
            # Start the queue runners.
                ...
                csv_file = open("./csv/alexnet_CPU_metrics_"+str(FLAGS.task_id)+".txt","w")
                ...
                for step in range(FLAGS.max_steps):
                    ...
                    memoryUse = pid_use.memory_info()[0]/2.**20  # memory use in MB...
...
#                _, loss_value, gs = sess.run([train_op, loss, global_step], feed_dict={batch_size: batch_size_num})
                    b = time.time()
                    # thread = threading.Thread(target=get_computation_time, name="get_computation_time",args=(run_metadata.step_stats,step,))
                    # thread.start()
